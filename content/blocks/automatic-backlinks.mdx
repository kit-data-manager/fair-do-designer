...
Blocks in this category are used to automatically create ***inverse** links* between related FAIR DOs.
...
This block is used to automatically create backlinks to another FAIR DO that has linked to this FAIR DO. This block expects the PID of the attribute to be linked to as input.

The attribute PID can be provided as text, but should be provided using the [Attribute PID](#attribute-pid) block.

> [!important]
>
> See the [Linking FAIR DOs guide](/docs/connecting-fair-dos) to understand the concept of inverse links.
...
This block returns the PID of the selected attribute. The block is linked to a fixed profile which is denoted by its name. The block shown above returns attribute PIDs for the Helmholtz Kernel Information Profile.
