...
![Explain a link and reverse link visually.](./img/connecting-fair-dos/term-reference-inverse.drawio.svg)

Linking within FAIR DOs is not a complicated mechanism:
It means to put the PID of one record as a value of another record, similar to Hyperlinks in the Web.

Profiles often provide attributes that require a PID as a value. Those are linking attributes.
In many cases, these will be optional, as a profile creator usually can't make assertions about the relations of a specific FAIR DO.
This does not mean they are not important: These relations show the way to
metadata,
creators,
related data,
or other context.
...
How can we link FAIR DOs in a Design? Let's go through usual use cases, step by step.
...
![Example 1 links two records per document together, in both directions.](./img/connecting-fair-dos/connect-from-same-document.drawio.svg)
...
But how do you link them if you cannot see them?
The answer needs to be, as always, in your input data.
But let's first look at the solution:

![Example 2 links records from different documents.](./img/connecting-fair-dos/connect-between-documents.drawio.svg)
...
![Example 3 mixes the approaches, demonstrating that link can target arbitrary records.](./img/connecting-fair-dos/connect-between-and-within-documents.drawio.svg)
...
![The general concept of linking records in the FAIR DO Designer.](./img/connecting-fair-dos/use.drawio.svg)


The figure above describes how to define and assign the pseudo-pids visually.
It also shows that inverse links can be declared using two blocks.
...
- Linking on both directions (Backlinks, see figure 1) are only possible for relations within the same document. To work around this, [Automatic backlinks](/docs/blocks/automatic-backlinks) exist.
